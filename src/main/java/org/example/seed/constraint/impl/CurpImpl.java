...
public class CurpImpl implements ConstraintValidator<Curp, String> {
    ...
    public boolean isValid(final String value, final ConstraintValidatorContext constraintValidatorContext) {
        return Optional.of(value)
                .map(v -> v.matches("^[A-Z]{4}[\\d]{6}(H|M){1}(AS|BC|BS|CC|CH|CL|CM|CS|DF|DG|GR|GT|HG|JC|MC|MN|MS|NE|NL|NT|OC|PL|QR|QT|SL|SP|SR|TC|TL|TS|VZ|YN|ZS){1}[A-Z]{3}([\\d]|[A-Z]){1}[\\d]{1}$"))
                .orElse(true);
    }
}
